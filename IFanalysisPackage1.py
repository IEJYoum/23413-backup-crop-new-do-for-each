...
#startFolder = r"C:\Users\youm\src"
#startFolder = r"Y:\Cyclic_Analysis\cmIF_2021-11-21_HTAN\Panel-A\kchin"
#startFolder = r"Y:\Cyclic_Workflow\cmIF_2020-09-15_SMT101Bx4\Segmentation"
#startFolder = "Y:/Cyclic_Workflow/cmIF_2020-10-08_SMT151/Segmentation"
#startFolder = r"Y:\Cyclic_Workflow\kchin\HER2B_2022-03_Dave/"
#startFolder = "C:/Users/youm/Desktop"
#startFolder = "Y:/ChinData/Cyclic_Analysis"
#startFolder = "Y:/Cyclic_Workflow"
#startFolder = "Y:\ChinData\Cyclic_Analysis\cmIF_2019-01-02_T-ATAC\9_13_T-ATAC_restore"
#startFolder = "Y:\Cyclic_Workflow\cmIF_2020-01-14_HTAN9\DataForIsaac"
#Y:\Cyclic_Workflow\cmIF_2022-02-04_HER2Bx4\Segmentation\HER2B-K173_CellposeSegmentation
 #  #r"C:\Users\youm\Desktop\old src"

STARTFOLDER = ""
HOMEFOLDER = ""
COMMANDLIST = ["1","4","","1"] #Load zz_amtec and move to analysis tool
COMPOS = 0
    
DATAFOLDER = r'W:\ChinData\Cyclic_Analysis\cmIF_2022-03-xx_mTMA2\GM'#r'Y:\Cyclic_Workflow'#r"C:\Users\youm\Desktop\src"#r"Y:\Cyclic_Analysis\cmIF_2019-10-04_MB-TMA\Chin_2019-10" #r'Y:\Cyclic_Workflow\cmIF_2019-12-02_JP-TMA1\Segmentation'#r"Y:\Cyclic_Workflow\cmIF_2021-11-21_HTAN-A\Segmentation\HTA9-14Bx1-7_CellposeSegmentation"
#DATAFOLDER = r"C:\Users\youm\Desktop\src"
#Y:\Cyclic_Workflow\cmIF_2021-11-21_HTAN-A\Segmentation\TNP-TMA-28_CellposeSegmentation    #r"Y:\Cyclic_Analysis\cmIF_2021-11-21_HTAN\Panel-A\kchin\afsubtracted"
def main(dataFolder=DATAFOLDER,saveFolder=r"C:\Users\youm\Desktop\src"):
    global STARTFOLDER,HOMEFOLDER,cm
    STARTFOLDER,HOMEFOLDER=dataFolder,saveFolder
    STARTFOLDER = dataFolder.replace("\ "[0],"/")
    HOMEFOLDER = saveFolder.replace("\ "[0],"/")
    print("main",HOMEFOLDER)
    df,obs,dfxy = loadingMenu() 
    print(all(obs.index==df.index),"all index the same")
    print(df.columns[df.isna().any()],"df.isna().any()")
    op = ["loading menu","analysis tool","Support Vector Machine"]
    fn = [loadingMenu,cm.main,sv.main]
    while True:
        ...
        if inp == "y" or inp == "":
            #break
            #print(df.head(),obs.head(),dfxy.head())
            print(df.index,'\n',obs.index,df.shape,obs.shape)
            print(all(df.index==obs.index))
            return(df,obs,dfxy)


def menu1(options,functions,df=9,obs=9,dfxy=9): #AUTOMENU
    global COMPOS
    print("menu")
    while True:
        print("\n")
...
def loadingMenu(df=9,obs=9,dfxy=9): 
    print(HOMEFOLDER)
    print("loading")
    if isinstance(df,type(9)):
        options = ["Import and clean data","load prepared data"]
        functions = [buildDataFrame,load]
        print("navigate to the folder that contains most of the data")
        df,obs,dfxy = menu(options,functions)
        print(df.shape,obs.shape,dfxy.shape)
    #df["index"],obs["index"],dfxy["index"] = df.index,df.index,df.index
    '''
    if input("drop duplicate indecies? (y)") == "y":
        df = df.drop_duplicates(subset=["index"],keep="last")
        obs = obs.drop_duplicates(subset=["index"],keep="last")
        dfxy = dfxy.drop_duplicates(subset=["index"],keep="last")
    '''
    if False:#input("standardize XY coordinate channels? (y)") == "y":
        xs = []
        ys = []
        ...
        X = xdf.max(axis=1)
        Y = ydf.max(axis=1)
        nxy = pd.DataFrame(index=dfxy.index)
        nxy["X"] = X
        nxy["Y"] = Y
        dfxy = nxy
    ...
    dfxy = dfxy.drop("index",axis=1)
    print(df.shape,obs.shape,dfxy.shape,"df,obs,xy\n\n\n\n")
    '''

    op = ["save","drop cells with less than N% of data","import data to fill missing values","fillNA values",
          ...
          "scale data (z-score, etc","autoclean NA values","edit observation labels"]
    fn = [save,dropCells,importMissing,fillNA,editObs,dropCols,importBiom,
          combineObs,saveObs,renCol,combineData,doPart,scale,autoClean,editLabels] 
    df,obs,dfxy = menu(op,fn,df,obs,dfxy)
    print(df.shape,obs.shape,dfxy.shape)
    return(df,obs,dfxy)
...
def renCol(df,obs,dfxy):
    ...
    if ch[0] == "Y" or ch[0] == "y":
        for i,d in enumerate(ds):
            ds[i] = d.loc[:,d.columns.sort_values()] #this apparently doesn't work
        return(ds[0],ds[1],ds[2])
    return(ds[0],ds[1],ds[2])
